...
export default function GroupExcerptEdit( {
	...
	if ( isProtected && ! userCanEdit ) {
		return (
			<div { ...blockProps }>
				<Warning>
					{ __(
						'There is no excerpt because this is a protected group.'
					) }
				</Warning>
			</div>
		);
	...
	const excerptContent = isEditable ? (
		<RichText
			...
			aria-label={ __( 'Group excerpt text' ) }
			value={
				isSelected
		...
		<p className={ excerptClassName }>
			{ ! isTrimmed
				? rawOrRenderedExcerpt || __( 'No group excerpt found' )
				: trimmedExcerpt + ELLIPSIS }
		</p>
	);
	return (
